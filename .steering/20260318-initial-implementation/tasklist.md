...
## Phase 6：コンテキストエンジニアリング

> Notionロードマップより追加。成果物：本番公開されたMVP

- [ ] UI全体の調整・余白・フォントサイズ統一
- [ ] バグ修正・全受け入れ条件の通しチェック
- [ ] ストレージ階層化（画像圧縮・CDNキャッシュ設定）
- [ ] GitHub リポジトリに push
- [ ] Vercel プロジェクト作成・GitHub 連携・環境変数設定
- [ ] デプロイ・公開 URL で動作確認

**完了条件：** 本番URLでMVPが公開され、全機能が動作する

---

## Phase 7：（未定義）

> Notionロードマップ上に存在しないフェーズ番号。必要に応じて追加。
...
- [ ] プロジェクトごとのロードマップ表示ページ実装
- [ ] ロードマップのステップ追加・編集・削除
- [ ] 進捗率の自動計算・表示
- [ ] ノートとロードマップの紐付け

**完了条件：** プロジェクトのロードマップが作成・管理できる
...
- [ ] Mermaid.js によるフロー図・ER図の描画
- [ ] ノートから図式を自動生成（Claude API 利用）
- [ ] 図式の保存・編集

**完了条件：** ノートの内容から図式が生成・表示できる
...
- [ ] ディスカッションルームの作成・参加
- [ ] Supabase Realtime によるリアルタイムチャット
- [ ] 招待リンクの生成・有効期限設定
- [ ] AI による会話履歴の要約

**完了条件：** 複数ユーザーがリアルタイムで議論でき、AIが要約を生成できる
