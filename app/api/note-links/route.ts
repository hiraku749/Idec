...
export async function GET(request: Request) {
  ...
  // 前方リンク（このノートが参照しているノート）
  const { data: forwardLinks } = await supabase
    .from('note_links')
    .select('target_note_id, notes!note_links_target_note_id_fkey(id, title)')
    .eq('source_note_id', noteId)
    .eq('user_id', user.id)

  // 後方リンク（このノートを参照しているノート）
  ...
  return NextResponse.json({
    forwardLinks: (forwardLinks ?? []).map((l) => ({
      noteId: l.target_note_id,
      title: (l.notes as unknown as { title: string })?.title ?? '無題',
    })),
